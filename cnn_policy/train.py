"""
CNN Policy Training Script
===========================
Train CNN for move prediction, matching CT-EFT-20 protocol.

Fair Comparison Requirements:
- Same dataset (LE22ct)
- Same training steps (100K)
- Same effective batch (2048 via gradient accumulation)
...
def get_vaswani_lr(step: int, d_model: int, warmup_steps: int) -> float:
    """
    Vaswani learning rate schedule (from "Attention is All You Need").
    Matches CT-EFT-20 exactly!
...
class LabelSmoothedCrossEntropyLoss(nn.Module):
    ...
    Matches CT-EFT-20's loss function.
    """
    ...
    def forward(self, logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        # Other classes get smoothing / (num_classes - 1)
...
class Trainer:
    """Trainer for CNN policy network."""
    ...
    def __init__(self, config):
        ...
        # Loss function (with label smoothing like CT-EFT-20)
        self.criterion = LabelSmoothedCrossEntropyLoss(smoothing=config.LABEL_SMOOTHING)
        
        # Optimizer (match CT-EFT-20 exactly!)
        ...
        if self.use_amp:
            ...
            print(f"   ⚡ Expected 2x speedup on H100!")
        
        # Data loaders
        print("\n📂 Loading datasets...")
        
        # Try H5 first (most fair), fallback to CSV
        h5_path = config.DATA_PATH / 'raw/LE22ct/LE22ct.h5'
        train_csv_path = config.DATA_PATH / 'processed/train.csv'
        
        if h5_path.exists():
            print(f"   Using H5 format (matches CT-EFT-20 exactly!)")
            ...
            print(f"   Path: {train_csv_path}")
            self.train_dataset = ChessPolicyDataset(train_csv_path, augment=False)
            self.val_dataset = ChessPolicyDataset(config.DATA_PATH / 'processed/val.csv', augment=False)
        else:
            raise FileNotFoundError(
                f"No dataset found!\n"
                f"  Tried H5: {h5_path}\n"
                f"  Tried CSV: {train_csv_path}\n"
                f"  Please download LE22ct or provide CSV files."
            )
    ...
    def train_step(self) -> Tuple[float, float, float]:
        ...
        Returns:
            (from_loss, to_loss, combined_loss)
        """
        self.model.train()
        ...
        for acc_step in range(Config.GRADIENT_ACCUMULATION_STEPS):
            ...
            # Backward (accumulate gradients) with gradient scaling
            if self.use_amp:
    ...
    def validate(self) -> Tuple[float, float, float]:
        ...
        Returns:
            (from_accuracy, to_accuracy, combined_accuracy)
        """
        self.model.eval()
    ...
    def train(self):
        """Main training loop (step-based, matching CT-EFT-20)."""
        print("\n" + "=" * 70)
        print("🚀 Starting Training (CT-EFT-20 Protocol)")
        print("=" * 70)
        print(f"\nTotal steps: {Config.TOTAL_STEPS:,}")
        print(f"Effective batch: {Config.BATCH_SIZE * Config.GRADIENT_ACCUMULATION_STEPS}")
        print(f"Expected epochs: ~15.4")
        print()
                ...
                for step in range(1, Config.TOTAL_STEPS + 1):
                    ...
                    if step % 5000 == 0 or step == Config.TOTAL_STEPS:
                        ...
                        if self.patience_counter >= Config.EARLY_STOPPING_PATIENCE:
                            ...
                            # Don't break - let user manually stop with Ctrl+C
    ...
    def finalize(self):
        ...
        print(f"   Time per 1K steps: {total_time / self.current_step * 1000 / 60:.1f} min")
        
        # Average checkpoints (like CT-EFT-20!)
        ...
        print(f"\n🎯 Final averaged model: {Config.CHECKPOINT_DIR / 'averaged_final.pth'}")
        print(f"   Best move accuracy: {self.best_val_accuracy:.2%}")
        print("\n" + "=" * 70)
...
def main():
    ...
    # Check data (H5 or CSV)
    h5_path = Config.DATA_PATH / 'raw/LE22ct/LE22ct.h5'
    csv_path = Config.DATA_PATH / 'processed/train.csv'
    
    if not h5_path.exists() and not csv_path.exists():
        print("❌ Training data not found!")
        print(f"\n   Tried H5: {h5_path}")
        print(f"   Tried CSV: {csv_path}")
        print("\n📋 Please download LE22ct dataset:")
        print("   python scripts/download_le22ct.py")
        print("\n   OR provide CSV files in:")
        print(f"   {csv_path.parent}/")
        sys.exit(1)
    
    if h5_path.exists():
        print(f"\n✅ Found H5 dataset: {h5_path}")
        print(f"   This matches CT-EFT-20 data usage exactly!")
    else:
        print(f"\n✅ Found CSV dataset: {csv_path}")
        print(f"   Make sure it was converted from LE22ct for fair comparison")
