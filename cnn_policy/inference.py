...
====================
Move prediction and game playing - MATCHES CT-EFT-20 exactly!

This implements the same inference logic as CT-EFT-20 for fair comparison.
"""
...
# Square name to index mapping (matching CT-EFT-20)
SQUARES = {
...
def topk_sampling(logits: torch.Tensor, k: int = 1) -> torch.Tensor:
    ...
    Matches CT-EFT-20's sampling function.
    
    Args:
        logits: (B, num_classes) - unnormalized scores
        k: Number of top choices to sample from
...
def is_pawn_promotion(board: chess.Board, move_uci: str) -> bool:
    ...
    Matches CT-EFT-20's check.
    
    Args:
        board: Current board state
        move_uci: Move in UCI format (without promotion suffix)
        
    Returns:
        True if this is a pawn promotion
    """
    if len(move_uci) < 4:
        return False
...
def predict_move(
    ...
    """
    Predict best move using CNN policy model.
    MATCHES CT-EFT-20 inference exactly!
    
    Args:
        model: Trained CNN policy model
        board: Current board position
        encoder: Position encoder
        device: torch device
        k: Top-k sampling parameter (1 = best move)
        
    Returns:
        Predicted move (chess.Move object)
    """
    model.eval()
    ...
    with torch.no_grad():
        ...
        # Convert to log probabilities (like CT-EFT-20!)
        ...
        # Remove promotion suffixes for matching (like CT-EFT-20)
        ...
        # Handle pawn promotion (always queen, like CT-EFT-20!)
...
def test_inference():
    ...
    print("🧪 Testing CNN Policy Inference\n")
    ...
    # Predict move
    print("Predicting best move...")
    predicted_move = predict_move(model, board, encoder, device, k=1)
    
    print(f"✅ Predicted: {predicted_move.uci()}")
    print(f"   Legal: {predicted_move in board.legal_moves}")
    print()
    
    # Get probabilities
    move_probs = get_move_probabilities(model, board, encoder, device)
    
    print("Top 5 moves by probability:")
    sorted_moves = sorted(move_probs.items(), key=lambda x: x[1], reverse=True)
    for i, (move, prob) in enumerate(sorted_moves[:5], 1):
        print(f"  {i}. {move}: {prob:.2%}")
